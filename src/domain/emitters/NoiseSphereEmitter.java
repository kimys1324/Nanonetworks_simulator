...
public class NoiseSphereEmitter extends SphereEmitter{

	Random _randomGaussianNoise;
	
	public NoiseSphereEmitter(double x, double y, double startTime, double endTime,
		...
		_randomGaussianNoise = new Random();
	}
	...
	double getNumParticles(double time) {
		if(time >= _startTime && time < _endTime){
			double random = _amplitude * _randomGaussianNoise.nextGaussian();
			return (int)(random);
		}	
		return 0;
	}
