...
public class RectangularSphereEmitter  extends SphereEmitter{

	int _period;
	int _timeOn;

	
	public RectangularSphereEmitter(double x, double y, double startTime, double endTime,
			double amplitude, double sphereRadius, double emitterRadius, double initV, boolean punctual, boolean concentrationEmitter, int period, int timeOn, String color) {
	...
	public String getDescription(){
		...
		s += "wave period (ns): " + _period + "\n";
		s += "wave emiting time (ns): " + _timeOn + "\n";
		return s;
	}
	
	double getNumParticles(double time) {
		if(time >= _startTime && time < _endTime){
			if( ((time - _startTime)% _period) < _timeOn  )
				return _amplitude;
		}	
		return 0;
	}
