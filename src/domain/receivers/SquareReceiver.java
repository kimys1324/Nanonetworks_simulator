...
public class SquareReceiver extends ParticleReceiver{

	double _halfSide;
	
	public SquareReceiver(String name, double x, double y, boolean absorb, boolean accumulate, double side) {
	...
	public int simpleCount(ArrayList<Particle> particlesList) {
		...
		while(it.hasNext()){ 
			p = it.next();
			if(p.getX() > _x+_halfSide) break;
			if(p.getX() >= _x-_halfSide) 
				if((p.getY() >= _y-_halfSide)&&(p.getY() <= _y+_halfSide)){
					count++;
					if(_absorb) {
						toDeleteList.add(p);
					}
	...
	public String getDescription() {
		...
		s += "receiver side (nm): " + 2*_halfSide + "\n"; 
		return s;
	}
